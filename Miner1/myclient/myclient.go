...
func InitAll() {
	...
	wallet.CreateWallet("E")
	wallet.CreateWallet("F")
	wallet.CreateWallet("G")
	wallet.CreateWallet("H")
	wallet.CreateWallet("I")
	wallet.CreateWallet("J")
...
func InitCoins() {
	...
	for i := 0; i < 10; i++ {
		address := test.WalletAddress(words[i])
		basetx := transaction.BaseTx(address, 100, true, "-")
		tp.PubTx = append(tp.PubTx, basetx)
		basetx = transaction.BaseTx(address, int64(10*(i+1)), false, "-")
		tp.PubTx = append(tp.PubTx, basetx)
	}
	tp.SaveFile()
	MineForOneRound()
...
func StartClient() {
	...
	TestAlive()
	addrsAlive = addresses.AlivePort()
	BroadCastNewTransaction("A", "B", 15, 2)
	RedisInit()
